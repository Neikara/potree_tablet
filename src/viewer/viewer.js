...
export class Viewer extends EventDispatcher {
	...
	constructor(domElement, args = {}) {
		...
		this.isTablet = window.matchMedia("(pointer: coarse)").matches;
	...
	loadGUI(callback) {
		...
		sidebarContainer.load(new URL(Potree.scriptPath + '/sidebar.html').href, () => {
			...
			const vrButtonResult = VRButton.createButton(this.renderer);
			const handleVRButton = (vrButton) => {
				if (vrButton == null) {
				...
				let element = vrButton.element || vrButton;
			...
			if (vrButtonResult instanceof Promise) {
				vrButtonResult.then(handleVRButton);
			} else {
				handleVRButton(vrButtonResult);
			}
	...
	update(delta, timestamp) {
		...
		if (!this.freeze) {
			// In VR, `domElement.clientHeight` is the size of the PC canvas, not the VR viewport.
			// We store the actual dimensions of the VR viewport so that the LOD calculation is correct.
			if (this.renderer.xr.isPresenting) {
				const dbgCam = new THREE.PerspectiveCamera();
				const xrCams = this.renderer.xr.getCamera(dbgCam);
				if (xrCams.cameras.length > 0) {
					const vp = xrCams.cameras[0].viewport;
					this.renderer._vrScreenWidth  = vp.width  || this.renderer.domElement.clientWidth;
					this.renderer._vrScreenHeight = vp.height || this.renderer.domElement.clientHeight;
				}
			} else {
				this.renderer._vrScreenWidth  = null;
				this.renderer._vrScreenHeight = null;
			}
	...
	renderVR() {
		...
		{ // render measurement overlays after pointclouds → always in first plan
			if (this.measuringTool && this.measuringTool.scene) {
				let mScene = this.measuringTool.scene;
				mScene.matrixAutoUpdate = false;
				mScene.matrix.copy(view);
				mScene.matrixWorld.copy(view);
